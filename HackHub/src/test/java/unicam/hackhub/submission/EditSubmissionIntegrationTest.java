...
public class EditSubmissionIntegrationTest {
    ...
    @Test
    void updateSubmission_Success_ShouldReturn200() throws Exception {
        ...
        SubmissionRequest oldSub = new SubmissionRequest("https://ciao.site");

        mockMvc.perform(post(path+hackathon.getId())
                        .header("Authorization","Bearer "+getToken(user))
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(oldSub)))
                .andExpect(status().isCreated())
                .andExpect(content().string("Submission added"));

        Hackathon a = hackathonRepository.findById(hackathon.getId()).get();
        assertThat(a.getSubmission(team.getName()).getUrl()).isEqualTo("https://ciao.site");

        SubmissionRequest newSub = new SubmissionRequest("https://new.site");

        mockMvc.perform(put(path+hackathon.getId())
                        .header("Authorization","Bearer "+getToken(user))
                        .contentType(MediaType.APPLICATION_JSON)
        ...
        Hackathon b = hackathonRepository.findById(hackathon.getId()).get();
        assertThat(b.getSubmission(team.getName()).getUrl()).isEqualTo("https://new.site");
    }

    @Test
    void updateSubmission_NoSubmissionPresent_ShouldReturn400() throws Exception {
        Hackathon hackathon = createTestHackathon(LocalDate.now().minusDays(1)
                ,new Period(LocalDate.now(),LocalDate.now().plusDays(1)),
                4,new HackathonStatus(HackathonStatus.HackathonStateType.PROGRESS));
        User user = createUser("Gino","gino@gino.it");
        ...
        mockMvc.perform(put(path+hackathon.getId())
                ...
                .andExpect(jsonPath("$.message").value("Team doesn't have a submission"));
    ...
    @Test
    void updateSubmission_InvalidURL_ShouldReturn400() throws Exception {
        Hackathon hackathon = createTestHackathon(LocalDate.now().minusDays(1)
                ,new Period(LocalDate.now(),LocalDate.now().plusDays(1)),
                4,new HackathonStatus(HackathonStatus.HackathonStateType.PROGRESS));
        User user = createUser("Gino","gino@gino.it");
        ...
        SubmissionRequest newSub = new SubmissionRequest("https://new");
        ...
        Hackathon b = hackathonRepository.findById(hackathon.getId()).get();
        assertThat(b.getSubmission(team.getName()).getUrl()).isEqualTo("https://ciao.site");
    }
